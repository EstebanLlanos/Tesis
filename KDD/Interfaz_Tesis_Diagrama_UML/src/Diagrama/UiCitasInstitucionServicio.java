...
import Diagrama.ControladorCitasInstitucionExamen;
...
* @author Esteban
*/
public class UiCitasInstitucionServicio {
    ...
    JLabel labelDepartamento, labelCiudad, labelAnioInicio, labelAnioFin, labelCriterioConsulta, labelMesInicio, labelMesFin, labelInstitucion;
    JTextField textFieldInstitucion;
    ...
    public UiCitasInstitucionServicio(JFrame ventanaPrincipal) {

        busquedaNombre = new JCheckBox("Buscar por nombre de la Institucion");
        ...
        labelInstitucion = new JLabel();
        inicializarJLabel(labelInstitucion, "Institucion:                       \n");
        
        textFieldInstitucion = new JTextField();
        textFieldInstitucion.setPreferredSize(new Dimension(170, 25));
        ...
        busquedaNombre.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                if (busquedaNombre.isSelected()) {
                    ...
                    textFieldInstitucion.setEnabled(true);
        ...
        autoCompletar = new AutoSuggestor(textFieldInstitucion, ventanaPrincipal, obtenerIntituciones(), Color.WHITE.brighter(), Color.BLUE, Color.RED, 0.85f);
    ...
    void hacerConsulta(ActionEvent evt) {
        ...
        ArrayList<String[]> citasPorInstitucion = controladorCitasEspecialista.getCitas(especialista, departamento, ciudad, anioInicio, mesInicio, mesFin, anioFin, criterioConsulta);

        if (citasPorInstitucion.isEmpty()) {
        ...
        } else if (citasPorInstitucion.get(0)[0].equals("Error")) {
        ...
        } else if (citasPorInstitucion.get(0)[0].equals("Error Fecha Año")) {
        ...
        } else if (citasPorInstitucion.get(0)[0].equals("Error Fecha Mes")) {
            ...
            try {
                ...
                for (int i = 0; i <= citasPorInstitucion.size() - 1; i++) {
                    instituciones.add(citasPorInstitucion.get(i)[0]);
                    citas.add(Integer.parseInt(citasPorInstitucion.get(i)[1]));
                }

                if (!citasPorInstitucion.isEmpty()) {
                    PieChart = new FXPieChart("Top Instituciones", instituciones, citas);
                    BarChart = new FXBarChart("Top Instituciones", "Instituciones", instituciones, "Citas", citas, "Citas Realizadas");
                    LineChart = new FXLineChart("Top Instituciones", "Instituciones", instituciones, "Citas", citas, "Citas Realizadas");
                } else {
    ...
    public ArrayList<String> obtenerIntituciones(){
        ...
        try {
            ...
            ResultSet tabla = sentencia.executeQuery("SELECT id_institucion, nombre_institucion "
                    + "FROM dim_institucion;");
            
            while (tabla.next()) {
                instituciones.add(tabla.getObject(1) + ", " + tabla.getObject(2));
            }
        } catch (Exception ex) {
    ...
    public JLabel getLabelInstitucion() {
        return labelInstitucion;
    }

    public void setLabelInstitucion(JLabel labelInstitucion) {
        this.labelInstitucion = labelInstitucion;
    }

    public JTextField getTextFieldInstitucion() {
        return textFieldInstitucion;
    }

    public void setTextFieldInstitucion(JTextField textFieldInstitucion) {
        this.textFieldInstitucion = textFieldInstitucion;
    }
