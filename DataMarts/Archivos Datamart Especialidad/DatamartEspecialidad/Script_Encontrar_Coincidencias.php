...
)
{
	...
	fwrite($archivoEncontradosBD, "Registros Encontrados en la tabla $nombre_tabla con las columnas $columnas_a_imprimir \n");
	fclose($archivoEncontradosBD);

	fclose($archivoOriginalesTodos);
	...
	while(!feof($handle1))
	{
		...
		if(isset($array_linea_a1[0])==true)
		{
			...
			$codigo_departamento=trim($array_linea_a1[0]);
		...
		if(isset($array_linea_a1[1])==true)
		{
			...
			$codigo_ciudad=trim($array_linea_a1[1]);
		...
		if(isset($array_linea_a1[2])==true)
		{
			...
			$fecha_actividad=trim($array_linea_a1[2]);
		...
		$lineaOriginalEnc=implode(",", $array_linea_a1);
		$archivoOriginalesTodos=fopen($pathOriginalesTodos, "a");
		fwrite($archivoOriginalesTodos, $lineaOriginalEnc."\n");
		fclose($archivoOriginalesTodos);
			...
			while($ultima_posicion<$numero_filas)
			{
				...
				else if(is_array($resultados) && count($resultados)>0 )
				{
					...
					foreach ($resultados as $key => $fila_actual) 
					{
						...
						if( ($codigo_ciudad == trim($fila_actual['cod_ciudad'])) && ($codigo_departamento == trim($fila_actual['departamento_ciudad'])) ){
								
							$id_ciudad = trim($fila_actual['id_ciudad']);

							$linea_a_escribir = "$id_ciudad,$fecha_actividad,$demografia,$vendedor,$cantidad";
		...
		else
		{
			...
			$linea_no_encontro_resultados="No encontro el valor $numero_identificacion_afiliado en la tabla.";
		    $archivoNoEncontradosBD=fopen($pathArchivoNoEncontrados, "a");
			fwrite($archivoNoEncontradosBD, $linea_no_encontro_resultados."\n");
