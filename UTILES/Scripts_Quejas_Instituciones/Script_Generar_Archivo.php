	...
	&& $_REQUEST['cantidad_registros']>0)
{
	...
	$pathArchivoSalida="destino/".$carpetaPropia."/"."Quejas_Instituciones".$fecha_archivo.".csv";
	...
	while($cont_linea_actual_archivo < $cont_registros_totales)
	{
		...
		*/
		...
		$query_contar_instituciones="";
		$query_contar_instituciones.="SELECT COUNT(*) as numero_instituciones FROM dim_institucion;";
		$resultado_query_contar_instituciones=$coneccionBD->consultar2_no_crea_cierra($query_contar_instituciones);

		$numero_instituciones=0;
		if(isset($resultado_query_contar_instituciones[0]['numero_instituciones'])==true)
		{
			$numero_instituciones=intval($resultado_query_contar_instituciones[0]['numero_instituciones']);
		}//fin if
		...
		$posicion_institucion_aleatoria_bd=intval(random_int(1, $numero_instituciones));
		$query_info_institucion_aleatorio="";
		$query_info_institucion_aleatorio.="SELECT id_institucion as id_institucion FROM dim_institucion LIMIT 1 OFFSET ".$posicion_institucion_aleatoria_bd."; ";
		$resultado_query_info_institucion_aleatorio=$coneccionBD->consultar2_no_crea_cierra($query_info_institucion_aleatorio);

		$institucion_seleccionada=0;

		if(isset($resultado_query_info_institucion_aleatorio[0]['id_institucion'])==true)
		{
			$institucion_seleccionada=intval($resultado_query_info_institucion_aleatorio[0]['id_institucion']);
		}//fin if

		if ($institucion_seleccionada == 0) {
			$institucion_seleccionada = intval(random_int(1, 158));
		}

		$linea_a_escribir="$fecha_seleccionada,$ciudad_seleccionada,$calificacion_seleccionada,$institucion_seleccionada,1";
